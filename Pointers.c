...
double storeDataAddress(void)
{
    ...
    // get the address of global variable j and store it in pointer
    // must use the & when accessing an address of a variable the *
    // is not needed when storing an address in a pointer
    ptr = &j;
...
void pointerToNull(void){
    ...
    // get the address of the global static variable j and store it in the pointer
    // must use the & operator when accessing an adderss of a variable 
    // the * is not needed when storing an address in a pointer 
    ptr = &j;

    // check if the pointer is null 
    // the * is not needed when accessing the address of a pointer
    if (ptr){
        printf("The pointer is not null %d\n", ptr);
    } else {
